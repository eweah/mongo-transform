...
class CLI extends require("readline").Interface {
  ...
  login() {
      ...
      this.question("Password: ", (passwrd) => {
        if (passwrd === "exit" || passwrd === "quit" || passwrd === "leave") {
          ...
          if (passwrd.length === 0) {
            ...
            this.isValid(this.regexes().passwordregex, passwrd) === false &&
            passwrd.length > 0
          ) {
    ...
    const onlogin = (user) => {
      ...
      authUser[`\x1b[35mDETAILS\x1b[0m`][`\x1b[37mAS OF NOW\x1b[0m`] =
        this.ontimemessage(user.created_at);
    ...
    this.on("login", () => {
      ...
      this.question("Phone Number: ", (phone) => {
        ...
        if (
          this.isValid(this.regexes().phoneregex, phone) === false &&
          phone.length !== 0
        ) {
          console.log(
            "\x1b[31m%s\x1b[0m",
        ...
        this.question("Password: ", (password) => {
          ...
          readable.on("end", () => {
            if (password.length === 0) {
              ...
              this.isValid(this.regexes().passwordregex, password) === false &&
              password.length > 0
            ) {
    ...
    if (args === "--load --months=") {
      ...
      if (isOk === true) {
        this.once("all-orders", (orders) => {
          ...
          let chosentime = orders.filter(
            (order) => this.onmonths(order.created_at) <= months
          );
          if (chosentime.length === 0) {
            return this.emit("error", {
              error: `No order was placed within the last ${months} ${this.pluralize(
                "day",
                months
              )}`,
            });
  ...
  ontimemessage(date) {
    ...
    if (years > 0) {
      message = `${years} ${this.pluralize(
        "year",
        years
      )} ${months} ${this.pluralize("month", months)} ${days} ${this.pluralize(
        "day",
        days
      )} ago.`;
    } else if (months > 0) {
      message = `${months} ${this.pluralize(
        "month",
        months
      )} ${days} ${this.pluralize("day", days)} ${hours} ${this.pluralize(
        "hour",
        hours
      )} ago.`;
    } else if (days > 0) {
      message = `${days} ${this.pluralize(
        "day",
        days
      )} ${hours} ${this.pluralize("hour", hours)} ${minutes} ${this.pluralize(
        "minute",
        minutes
      )} ago.`;
    } else if (hours > 0) {
      message = `${hours} ${this.pluralize(
        "hour",
        hours
      )} ${minutes} ${this.pluralize(
        "minute",
        minutes
      )} ${seconds} ${this.pluralize("second", seconds)} ago.`;
    } else {
      message = `${hours} ${this.pluralize(
        "hour",
        hours
      ...
      for (let iorder of orders.sort()) {
        ...
        if (years > 0) {
          message = `${years} ${this.pluralize(
            "year",
            years
          )} ${months} ${this.pluralize(
            "month",
            months
          )} ${days} ${this.pluralize("day", days)} ago.`;
        } else if (months > 0) {
          message = `${months} ${this.pluralize(
            "month",
            months
          ...
          message = `${days} ${this.pluralize(
            "day",
            days
          )} ${hours} ${this.pluralize(
            "hour",
            hours
          )} ${minutes} ${this.pluralize("minute", minutes)} ago.`;
        } else if (hours > 0) {
          message = `${hours} ${this.pluralize(
            "hour",
            hours
        ...
        disp[`\x1b[35mORDER ${counter}\x1b[0m`]["\x1b[37mORDER ID #\x1b[0m"] =
          order.id;
        disp[`\x1b[35mORDER ${counter}\x1b[0m`]["\x1b[37mDATE\x1b[0m"] =
          order.created_at;
        disp[`\x1b[35mORDER ${counter}\x1b[0m`]["\x1b[37mAS FOR NOW\x1b[0m"] =
          message;
      }

      let string =
        typeof title === "object" ? ` ${title.message} ${time.message}` : title;
      let options;
      options = {
        string,
        number: process.stdout.columns,
  ...
  ondisplayusers(event) {
    return this.once(event, (users, title = "", time = "") => {
      ...
      for (let user of users.sort()) {
        ...
        disp[`\x1b[35mUSER ${counter}\x1b[0m`]["\x1b[37mPHONE\x1b[0m"] =
          user.phone;
        disp[`\x1b[35mUSER ${counter}\x1b[0m`]["\x1b[37mJOINED ON\x1b[0m"] =
          user.created_at;
        disp[`\x1b[35mUSER ${counter}\x1b[0m`]["\x1b[37mAS OF NOW\x1b[0m"] =
          message;
      }
  ...
  ondisplayuser(event) {
    return this.once(event, (user) => {
      ...
      if (user.records && !user.options) {
        let string = user.code
          ? ` USER ASSOCIATED WITH ${user.code.toUpperCase()} (ORDER RECORDS FOUND)`
          : ` USER ASSOCIATED WITH ${user.phone} (ORDER RECORDS FOUND)`;
        options = {
          string,
          number: process.stdout.columns,
          color: 36,
        ...
        let string = user.code
          ? ` USER ASSOCIATED WITH ${user.code.toUpperCase()} (ORDER RECORDS FOUND)`
          : ` USER ASSOCIATED WITH ${user.phone} (NO ORDER RECORDS)`;
        options = {
          string,
          number: process.stdout.columns,
          color: 36,
      ...
      data[`\x1b[35mDETAILS\x1b[0m`]["\x1b[37mJOINED ON\x1b[0m"] =
        user.created_at;
      ...
      if (!user.records) {
        if (user.options && user.options.length > 0) {
          for (let option of user.options) {
            if (option === "o") {
              let string = user.code
                ? ` ALL ORDERS (${user.code.toUpperCase()})`
                : ` ALL ORDERS (${user.phone})`;
              let ordersoptions = {
                string,
                number: process.stdout.columns,
                color: 36,
              ...
              if (
                user.jsonned &&
                user.jsonned.json !== undefined &&
                user.jsonned.depth !== undefined
              ) {
            ...
            if (option === "r") {
              let string = user.code
                ? ` ADDRESSES (${user.code.toUpperCase()})`
            ...
            if (option === "b") {
              let string = user.code
                ? ` BILLING ADDRESS (${user.code.toUpperCase()})`
                : ` BILLING ADDRESS (${user.phone})`;
              let billingoptions = {
                string,
                number: process.stdout.columns,
                color: 36,
      ...
      if (user.data && user.data.length > 0) {
        ...
        for (let order of user.data) {
          ...
          orders[`\x1b[35mORDER ${counter}\x1b[0m`][
            "\x1b[37mORDER ID #\x1b[0m"
          ] = order.id;
          orders[`\x1b[35mORDER ${counter}\x1b[0m`]["\x1b[37mDATE\x1b[0m"] =
            order.created_at;
          orders[`\x1b[35mORDER ${counter}\x1b[0m`][
            "\x1b[37mAS FOR NOW\x1b[0m"
          ] = message;
          counter++;
        }
      }
      ...
      if (user.records || (user.data && user.data.length > 0)) {
        if (user.records) {
          ...
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mADDRESS\x1b[0m"] =
            user.records.billing.billing_address;
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mCITY\x1b[0m"] =
            user.records.billing.billing_city;
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mSTATE\x1b[0m"] =
            user.records.billing.billing_state;
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mZIP\x1b[0m"] =
            user.records.billing.billing_zip;
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mPHONE\x1b[0m"] =
            user.records.billing.billing_phone;
          records[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mEMAIL\x1b[0m"] =
            user.records.email;

          records[`\x1b[35mSHIPPING\x1b[0m`] = [];
          //records[`\x1b[35mSHIPPING\x1b[0m`][`\x1b[35mSHIPPING\x1b[0m`] = []
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mADDRESS\x1b[0m"] =
            user.records.address;
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mCITY\x1b[0m"] =
            user.records.city;
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mSTATE\x1b[0m"] =
            user.records.state;
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mZIP\x1b[0m"] =
            user.records.zip;
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mPHONE\x1b[0m"] =
            user.records.phone;
          records[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mEMAIL\x1b[0m"] =
            user.records.email;
          ...
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mADDRESS\x1b[0m"] =
            user.records.address;
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mCITY\x1b[0m"] =
            user.records.city;
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mSTATE\x1b[0m"] =
            user.records.state;
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mZIP\x1b[0m"] =
            user.records.zip;
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mPHONE\x1b[0m"] =
            user.records.phone;
          shipping[`\x1b[35mSHIPPING\x1b[0m`]["\x1b[37mEMAIL\x1b[0m"] =
            user.records.email;
          ...
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mADDRESS\x1b[0m"] =
            user.records.billing.billing_address;
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mCITY\x1b[0m"] =
            user.records.billing.billing_city;
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mSTATE\x1b[0m"] =
            user.records.billing.billing_state;
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mZIP\x1b[0m"] =
            user.records.billing.billing_zip;
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mPHONE\x1b[0m"] =
            user.records.billing.billing_phone;
          billing[`\x1b[35mBILLING\x1b[0m`]["\x1b[37mEMAIL\x1b[0m"] =
            user.records.email;

          jsonbilling[`BILLING`] = [];
          //jsonbilling[`BILLING`][`BILLING`] = []
          jsonbilling[`BILLING`]["ADDRESS"] =
            user.records.billing.billing_address;
        ...
        if (user.options && user.options.length > 0) {
          for (let option of user.options) {
            ...
            if (option === "s") {
              let string = user.code
                ? ` SHIPPING ADDRESS (${user.code.toUpperCase()})`
                : ` SHIPPING ADDRESS (${user.phone})`;
              let shippingoptions = {
                string,
                number: process.stdout.columns,
                color: 36,
  ...
  validateTimeEvent(event) {
    this.timeEventList().find((_event) => _event === event.trim()) !== undefined
      ? true
      : false;
  }
  jsonEventList() {
  ...
  validateJsonEvent(event) {
    return this.jsonEventList().find((_event) => _event === event.trim()) !==
      undefined
      ? true
      : false;
  }
  ...
  byTimeEvent(event) {
    ...
    if (_event !== undefined) {
      ...
      return this.emit("error", {
        error: `Interval error: Not found in event list `,
      });
    }
  }
  byTime(event) {
  ...
  getOrderByTime(orders, event, data) {
    ...
    if (longcode !== undefined && shortcode !== undefined) {
      ...
      if (type && type !== undefined) {
        title = {
          message: `ALL THE ORDERS ${type.toLocaleUpperCase()} PLACED WITHIN THE LAST`,
        };
      } else {
        title = { message: `ALL ORDERS WITHIN THE LAST` };
      }
      let chosenorders = orders.filter(
        (order) =>
          this.eventMethodFromString(event)(order.created_at) <= timer.value
      );
      if (type && type !== undefined) {
        if (chosenorders.length === 0) {
          return this.emit("orders:info", {
            error: `${type} placed no order within the last ${
              timer.value
            } ${this.pluralize(this.byTime(event), timer.value)}`,
          });
        }
      } else {
        if (chosenorders.length === 0) {
          return this.emit("orders:info", {
            error: `No order was placed within the last ${
              timer.value
            } ${this.pluralize(this.byTime(event), timer.value)}`,
          });
        }
      ...
      let time = {
        message: `${timer.value} ${this.pluralize(
          this.byTime(event),
          timer.value
        ).toLocaleUpperCase()}`,
      };
